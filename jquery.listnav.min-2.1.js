...
* Author: Jack Killpatrick, with contributions from Alexander Mashin
*
* Version 2.2 (28/07/2019)
...
(function($) {
	$.fn.listnav = function(options) {
		...
		return this.each(function() {
			...
			function init() {
				var letters = addClasses ();
				$wrapper.append(createLettersHtml(letters));
				$letters = $('.ln-letters', $wrapper).slice(0, 1); // will always be a single item
				if (opts.showCounts) $letterCount = $('.ln-letter-count', $wrapper).slice(0, 1); // will always be a single item

				addNoMatchLI();
				if (opts.flagDisabled) addDisabledClass(letters);
				bindHandlers();
